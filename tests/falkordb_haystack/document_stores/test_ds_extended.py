...
def test_document_store_connection_parameters():
    ...
    doc_store = FalkorDBDocumentStore(**connection_config, **skip_db_interactions_for_test)

    assert doc_store.client_config.url == "bolt://db:7687"
    assert doc_store.client_config.database == "database"

    client_config = FalkorDBClientConfig(**connection_config)
    doc_store = FalkorDBDocumentStore(client_config=client_config, **skip_db_interactions_for_test)

    assert doc_store.client_config == client_config


@pytest.mark.integration
def test_get_all_documents_generator(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
    assert len(list(doc_store.get_all_documents_generator(batch_size=2))) == 9


@pytest.mark.integration
def test_get_all_documents_without_embeddings(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_get_document_by_id(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_get_documents_by_id(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_count_documents_with_filter(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_get_embedding_count(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    """
...
@pytest.mark.integration
def test_delete_all_documents(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_delete_documents_with_filters(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_delete_documents_by_id(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_delete_documents_by_id_with_filters(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
    docs_to_delete = doc_store.filter_documents(filters={"field": "year", "operator": "==", "value": 2020})
...
@pytest.mark.integration
def test_delete_index(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
    ...
    assert doc_store.count_documents() == 0
    assert (
        doc_store.falkordb_client.retrieve_vector_index(doc_store.index, doc_store.node_label, doc_store.embedding_field)
        is None
    )


@pytest.mark.integration
def test_delete_index_does_not_raise_if_not_exists(doc_store_factory: Callable[..., FalkorDBDocumentStore]):
    """By default FalkorDB will trigger DatabaseError (server) if trying to remove index which does not exist"""
    doc_store = doc_store_factory(index="document-embeddings", create_index_if_missing=False)
    doc_store.delete_index()


@pytest.mark.integration
def test_get_all_documents_large_quantities(doc_store: FalkorDBDocumentStore):
    docs_to_write = [
        Document.from_dict(
...
@pytest.mark.integration
def test_update_meta(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
    doc = documents[0]

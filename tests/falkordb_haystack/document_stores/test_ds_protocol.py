...
def test_write_documents(doc_store: FalkorDBDocumentStore, documents: List[Document]):
...
@pytest.mark.integration
def test_write_with_duplicate_doc_ids(doc_store: FalkorDBDocumentStore):
    duplicate_documents = [
...
@pytest.mark.integration
def test_count_documents(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_filter_documents_duplicate_text_value(doc_store: FalkorDBDocumentStore):
    documents = [
...
@pytest.mark.integration
def test_filter_documents_with_correct_filters(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_filter_documents_with_incorrect_filter_name(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_filter_documents_with_incorrect_filter_value(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_duplicate_documents_skip(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_duplicate_documents_overwrite(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_duplicate_documents_fail(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@pytest.mark.integration
def test_write_document_meta(doc_store: FalkorDBDocumentStore):
...
@pytest.mark.integration
def test_delete_documents(doc_store: FalkorDBDocumentStore, documents: List[Document]):
    doc_store.write_documents(documents)
...
@mock.patch.object(FalkorDBClientConfig, "to_dict")
@mock.patch.object(FalkorDBClient, "verify_connectivity")
def test_document_store_to_dict(verify_connectivity_mock, to_dict_mock):
    ...
    assert data == {
        "type": "falkordb_haystack.document_stores.falkordb_store.FalkorDBDocumentStore",
        "init_parameters": {
            "index": "document-embeddings",
            "node_label": "Document",
            "embedding_dim": 384,
...
def test_document_store_from_dict(verify_connectivity_mock, from_dict_mock):
    ...
    expected_client_config = FalkorDBClientConfig(url="bolt://localhost:7687")
    from_dict_mock.return_value = expected_client_config

    doc_store = FalkorDBDocumentStore.from_dict(data)

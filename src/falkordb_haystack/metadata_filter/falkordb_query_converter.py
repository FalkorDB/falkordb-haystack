...
from falkordb_haystack.metadata_filter.parser import (
    AST,
    COMPARISON_OPS,
    LOGICAL_OPS,
    ComparisonOp,
...
class FalkorDBQueryConverter(NodeVisitor):
    """
    ...
    filters into FalkorDB Cypher expressions. Resulting Cypher query is then used as part of the ``WHERE`` FalkorDB clause and
    is based on the following concepts:

    - [WHERE clause](https://falkordb.com/docs/cypher-manual/current/clauses/where/)
    - [Comparison operators](https://falkordb.com/docs/cypher-manual/current/syntax/operators/#query-operators-comparison)
    - [Working with null](https://falkordb.com/docs/cypher-manual/current/values-and-types/working-with-null/)

    below is an example usage of the converter:

    ```python
    parser = FilterParser()
    converter = FalkorDBQueryConverter("doc")
    ...
    The above code will produce the following FalkorDB Cypher query (``cypher_query``):
    ...
    values to FalkorDB Python Driver instead of repeating the logic in this class**. See the full mapping of core and
    extended types in the [Data Types](https://falkordb.com/docs/api/python-driver/current/api.html#data-types) document.
    ...
    def __init__(
        ...
        """
        Constructs a new `FalkorDBQueryConverter` instance.
    ...
    def convert(self, op_tree: AST) -> Tuple[str, Dict[str, Any]]:
        ...
        Examples:
            ...
            >>> FalkorDBQueryConverter("n").convert(op_tree)
    ...
    def _field_is_null_expr(self, param: CypherFieldParam) -> str:
        ...
        Additional "IS NULL" checks can be useful for non-equality checks (e.g. ``"doc.age <> 0"``) as by default FalkorDB
        skips properties with null values, see [Operators Equality](https://falkordb.com/docs/cypher-manual/current/syntax/operators/#_equality)
    ...
    def _op_exists(self, param: CypherFieldParam) -> CypherQueryExpression:
        ...
        absent properties. See more details in FalkorDB documentation:

        - [Filter on `null`](https://falkordb.com/docs/cypher-manual/current/clauses/where/#filter-on-null)
        - [Property existence checking](https://falkordb.com/docs/cypher-manual/current/clauses/where/#property-existence-checking)
        ...
        Returns:
            Cypher expression to check if property is ``null`` (property existence in FalkorDB)
        """
    ...
    def _op_in(self, param: CypherFieldParam) -> CypherQueryExpression:
        ...
        See more details in FalkorDB documentation:

        - [IN operator](https://falkordb.com/docs/cypher-manual/current/clauses/where/#where-in-operator)
        - [Conditional expressions](https://falkordb.com/docs/cypher-manual/current/queries/case/)
        - [Function ``any()``](https://falkordb.com/docs/cypher-manual/current/functions/predicate/#functions-any)
        ...
        would not work in FalkorDB. Thus ``CASE`` checks the type of the property and if its a list ``any()`` function
    ...
    def _op_nin(self, param: CypherFieldParam) -> CypherQueryExpression:
        ...
        documentation of the [_op_in][falkordb_haystack.metadata_filter.falkordb_query_converter.FalkorDBQueryConverter._op_in]
        method for more details.

        Additional "IS NULL" expression will be added if such configuration is enabled. See implementation of
            [_field_is_null_expr][falkordb_haystack.metadata_filter.falkordb_query_converter.FalkorDBQueryConverter._field_is_null_expr].
    ...
    def _op_default(self, param: CypherFieldParam) -> CypherQueryExpression:
        ...
        The mapping between metadata filter operators and FalkorDB operators is stored in `self.COMPARISON_MAPPING`.
    ...
    def _normalize_field_type(self, field_value: FieldValueType) -> FieldValueType:
        ...
        to `falkordb` python driver. This way we reduce amount of logic in the converter and rely on the driver to make
        sure field value types are properly mapped between Python and FalkorDB. In some cases though we can handle
        ...
        `:::cypher doc.age IN [20, 30, 40]`. However falkordb python driver does not accept `set` python type, thus we
        convert it to `list` to make such filters possible.

        Args:
            field_value: Field value to be adjusted to be compatible with FalkorDB types.

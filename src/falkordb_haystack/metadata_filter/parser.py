...
from falkordb_haystack.errors import FalkorDBFilterParserError
    ...
    * more complex expressions like ``"(field1 = 1 OR field2 < 4) AND field3 > 2"``
    ...
    def __init__(self, operands: Sequence[AST], op: str):
        if OpType.from_op(op) != OpType.LOGICAL:
            raise FalkorDBFilterParserError(
...
class FilterParser:
    ...
    def _parse_comparison_op(self, filters: FilterType) -> ComparisonOp:
        ...
        Raises:
            FalkorDBFilterParserError: If required `field` or `value` comparison dictionary keys are missing.
        ...
        if not field_name:
            raise FalkorDBFilterParserError(f"`field` is mandatory in comparison filter dictionary: `{filters}`.")

        if filter_value is None:
            raise FalkorDBFilterParserError(f"`value` is mandatory in comparison filter dictionary: `{filters}`.")
    ...
    def _parse_logical_op(self, filters: FilterType) -> LogicalOp:
        ...
        Raises:
            FalkorDBFilterParserError: If required `conditions` logic dictionary key is missing or is not a `list` with at
                least one item.
        ...
        if not conditions or not isinstance(conditions, list) or len(conditions) < 1:
            raise FalkorDBFilterParserError("Can not parse logical operator with empty or absent conditions")
    ...
    def _parse_tree(self, filters: FilterType) -> OperatorAST:
        ...
        Raises:
            FalkorDBFilterParserError: If required `operator` dictionary key is missing.
            FalkorDBFilterParserError: If `filters` is not a dictionary.
            FalkorDBFilterParserError: If operator value is unknown.
        ...
        if not isinstance(filters, dict):
            raise FalkorDBFilterParserError("Filter must be a dictionary.")

        if "operator" not in filters:
            raise FalkorDBFilterParserError("`operator` must ne present in both comparison and logic dictionaries.")
    ...
    def parse(self, filters: FilterType) -> OperatorAST:
        ...
        Raises:
            FalkorDBFilterParserError: In case parsing results in empty `AST` tree or more than one root node has been

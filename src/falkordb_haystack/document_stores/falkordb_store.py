...
from falkordb_haystack.metadata_filter import COMPARISON_OPS, FilterParser, OperatorAST
from falkordb_haystack.serialization.types import QueryParametersMarshaller
...
class FalkorDBDocumentStore:
    """
    Document store for [FalkorDB Database](https://falkordb.com/) with support for dense retrievals using \
    [Vector Search Index](https://falkordb.com/docs/cypher-manual/current/indexes-for-vector-search/)

    The implementation is based on [Python Driver](https://falkordb.com/docs/python-manual/current/) for database access.
    ...
    The following should be expected after writing documents to FalkorDB (see `FalkorDBDocumentStore.write_documents`).

    ```json title="FalkorDB node json representation for a document (with comments)"
    {
      "identity": 18900, // FalkorDB native id
    ...
    Please notice the `embedding` property which is stored as part of FalkorDB node properties. It has type ``LIST<FLOAT>``
    ...
    `FalkorDBDocumentStore`. The index is created using `db.index.vector.createNodeIndex()` FalkorDB procedure and is based
    on the `embedding` property.

    Embedding dimension as well as similarity function (e.g. `cosine`) are configurable.
    At the moment FalkorDB supports only cosine and euclidean(l2) similarity functions.

    Metadata filtering by `FalkorDBDocumentStore` is performed using the standard `WHERE` Cypher query clause.
    Vector search is implemented by calling `db.index.vector.queryNodes()` procedure. **FalkorDB currently does not support
    ...
    [Indexes for search performance](https://falkordb.com/docs/cypher-manual/current/indexes-for-search-performance/).
    It can be managed directly in FalkorDB as an administrative task.

    You have several options available for deploying/installing FalkorDB. See more details in \
    [Installation Operations Manual](https://falkordb.com/docs/operations-manual/current/installation/).
    As of FalkorDB 5.13, the vector search index is no longer a beta feature.
    ...
    # Create `FalkorDBDocumentStore` with required credentials and Vector index configuration
    document_store = FalkorDBDocumentStore(
        ...
        database="falkordb",
        embedding_dim=384,
        index="document-embeddings", # The name of the Vector search index in FalkorDB
        node_label="Document", # Providing a label to FalkorDB nodes which store Documents
    )

    # Write documents to FalkorDB. Respective nodes will be created.
    ...
    def __init__(
        ...
        falkordb_client: Optional[FalkorDBClient] = None,
    ):
        """
        Constructor method
        ...
        Args:
            ...
            client_config: Advanced client configuration to control various settings of underlying falkordb python
                driver. See `FalkorDBClientConfig` for more details. The mandatory `url` attribute will be set on
                the `client_config` in case it was provided in the config itself.
            index: The name of FalkorDB Vector Search Index used for storing and querying embeddings.
            node_label: The name of the label used in FalkorDB to represent `haystack.Document`.
                FalkorDB nodes are used primarily as storage for Document attributes and metadata filtering.
            ...
            embedding_field: the name of embedding field which is created as a FalkorDB node property containing an
            ...
            document_marshaller: A custom marshaller, if provided, to convert `haystack.Document` to a dictionary to be
                stored as FalkorDB node properties. **FalkorDB can not store nested properties in a node** so this
        ...
        if verify_connectivity:
            self.falkordb_client.verify_connectivity()

        if recreate_index:
            self.delete_index()
            self.create_index()
        elif create_index_if_missing:
            self.falkordb_client.create_index_if_missing(
    ...
    def write_documents(
        ...
        Args:
            documents: List of `haystack.Document`. If they already contain the embeddings, we'll index
                them right away in FalkorDB. If not, you can later call `update_embeddings` to create and index them.
        ...
        with tqdm(
            ...
            for document_batch in batched_documents:
                records = [self._document_to_falkordb_record(doc) for doc in document_batch]
                embedding_field = self.embedding_field
                self.falkordb_client.merge_nodes(self.node_label, embedding_field, records)
    ...
    def update_embeddings(
        ...
        with tqdm(
            ...
            for document_batch in get_batches_from_generator(documents, batch_size):
                ...
                self.falkordb_client.update_embedding(
                    self.node_label,
    ...
    def get_all_documents_generator(
        ...
        supported by underlying FalkorDB Python Driver (an internal buffer which is depleted while being read and filled
        ...
        Args:
            ...
            batch_size: When working with large number of documents, batching can help reduce memory footprint.
                This parameter controls how many documents are retrieved at once from FalkorDB.
        ...
        records = self.falkordb_client.find_nodes(self.node_label, filter_ast, skip_properties, fetch_size=batch_size)

        return (self._falkordb_record_to_document(rec) for rec in records)
    ...
    def get_documents_by_id(
        ...
        for batch_ids in get_batches_from_generator(document_ids, batch_size):
            ...
            records = self.falkordb_client.find_nodes(self.node_label, filter_ast)
            documents.extend([self._falkordb_record_to_document(rec) for rec in records])
    ...
    def count_documents_with_filter(
        ...
        return self.falkordb_client.count_nodes(self.node_label, filter_ast)
    ...
    def query_by_embedding(
        ...
        records = self.falkordb_client.query_embeddings(
            self.index, top_k, query_embedding, filter_ast, skip_properties, expand_top_k
        )
        results = [self._falkordb_record_to_document(rec) for rec in records]
    ...
    def delete_all_documents(
        ...
        self.falkordb_client.delete_nodes(self.node_label, filter_ast)
    ...
    def delete_index(self):
        ...
        try:
            self.falkordb_client.delete_index(self.index)
    ...
    def create_index(self):
        self.falkordb_client.create_index(
            self.index, self.node_label, self.embedding_field, self.embedding_dim, self.similarity_function
        )

    def update_document_meta(self, document_id: str, meta: Dict[str, Any]):
        """
        Updates metadata properties in FalkorDB for a Document found by its `document_id`. Please see details on how
        properties in nodes are being mutated in FalkorDB for a given `meta` dictionary (https://falkordb.com/docs/cypher-manual/current/clauses/set/#set-setting-properties-using-map)

        Args:
            document_id: The Document id to update in FalkorDB
            meta: Dictionary of new metadata. Will replace property values in case those already exist in the
                corresponding FalkorDB node. Please notice it is assumed Document metadata has same schema (e.g. same
                amount of properties and its names) as in originally created nodes in FalkorDB. Otherwise some side effects
                might be produced (e.g. a property is renamed in `meta` which leaves an old one in FalkorDB).
        """

        updated_record = self.falkordb_client.update_node(self.node_label, document_id, meta)
    ...
    def _get_distance(self, similarity: str) -> SimilarityFunction:
        """
        Validates similarity function so that it is supported by falkordb vector index.
        ...
        Raises:
            ValueError: If given similarity is not supported by falkordb.

        Returns:
            Similarity function supported by FalkorDB vector index ("cosine" or "euclidean").
        """
        try:
    ...
    def _parse_filters(
        ...
        *,
        ...
        """
        Utility method which combines different filters in order to build a final one to be sent to `FalkorDBClient`
    ...
    def _falkordb_record_to_document(self, record: FalkorDBRecord) -> Document:
        """
        Creates `Document` from FalkorDB record (`dict`).
        """
        return Document.from_dict(record)

    def _document_to_falkordb_record(self, document: Document) -> FalkorDBRecord:
        """
        Creates FalkorDB record (`dict`) from a `Document` using a configured marshaller. Please notice `meta` fields will
    ...
    def __del__(self):
        self.falkordb_client.close_driver()


class _DefaultDocumentMarshaller(QueryParametersMarshaller):
    """
    Default marshaller to convert `haystack.Document` before storing it in FalkorDB node.
    ...
    Please take a look at \
            [Property, structural, and constructed values](https://falkordb.com/docs/cypher-manual/current/values-and-types/property-structural-constructed/)
            to better understand what values can be stored as properties.

    **TODO: The logic can be improved so that nested attributes get stored as relations and nodes in FalkorDB**
    """
    ...
    def marshal(self, document: Document) -> Any:
        ...
        Args:
            document: The document to be converted to a FalkorDB record.
    ...
    def _exclude_non_serializable_objects(self, doc_object: Dict[str, Any]):
        ...
        for prop_name in obj_keys:
            if isinstance(doc_object[prop_name], (list, set, tuple)) and len(doc_object[prop_name]) > 0:
                ...
                is_simple = self._is_simple_falkordb_type(first_value)
                is_homogeneous = True
                ...
                if not is_simple or not is_homogeneous:
                    ...
                    logger.warning(
                        ...
                        f"Property `{prop_name}` will be skipped and not stored in FalkorDB node"
                    )
        return doc_object

    def _is_simple_falkordb_type(self, obj: Any):

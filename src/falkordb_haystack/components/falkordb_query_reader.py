...
from falkordb_haystack.serialization import (
    FalkorDBQueryParametersMarshaller,
...
class QueryResult(TypedDict):
    """
    Query execution outputs for the `FalkorDBQueryReader` component.
    """
...
@component
class FalkorDBQueryReader:
    """
    A component for reading arbitrary data from FalkorDB database using plain Cypher query.

    This component gives flexible way to read data from FalkorDB by running custom Cypher query along with query
    parameters. Query parameters can be supplied in a pipeline from other components (or pipeline inputs).
    You could use such queries to read data from FalkorDB to enhance your RAG pipelines. For example a
    prompt to LLM can produce Cypher query based on given context and then `FalkorDBQueryReader` can be used to run the
    query and extract results. [OutputAdapter](https://docs.haystack.deepset.ai/docs/outputadapter) component might
    become handy in such scenarios - it can be used as a connection from the `FalkorDBQueryReader` to convert (transform)
    results accordingly.

    Note:
        Please consider [data types mappings](https://falkordb.com/docs/api/python-driver/current/api.html#data-types) in \
        Cypher query when working with query parameters. FalkorDB Python Driver handles type conversions/mappings.
        Specifically you can figure out in the documentation of the driver how to work with temporal types.

    ```py title="Example: Find a Document node with FalkorDBQueryReader and extract data"
    ...
    client_config = FalkorDBClientConfig("bolt://localhost:7687", database="falkordb", username="falkordb", password="passw0rd")

    reader = FalkorDBQueryReader(client_config=client_config, runtime_parameters=["year"])
    ...
    ```py title="Example: Output error with FalkorDBQueryReader"
    from falkordb_haystack.client.falkordb_client import FalkorDBClientConfig
    from falkordb_haystack.components.falkordb_query_reader import FalkorDBQueryReader
    ...
    When configuring [Query parameters](https://falkordb.com/docs/python-manual/current/query-simple/#query-parameters) \
    for `FalkorDBQueryReader` component, consider the following:
    ...
    Important:
        ...
        `FalkorDBQueryReader(client_config=client_config).run(parameters={"obj": <instance of custom class>})` it will
    ...
    def __init__(
        ...
        """
        Creates a FalkorDBDocumentReader component.
        ...
        Args:
            ...
            query_parameters_marshaller: Marshaller responsible for converting query parameters which can be used in
                Cypher query, e.g. python dataclasses to be converted to dictionary. `FalkorDBQueryParametersMarshaller`
        ...
        self._query_parameters_marshaller = query_parameters_marshaller or FalkorDBQueryParametersMarshaller()
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "FalkorDBQueryReader":
        """
    ...
    def run(self, query: Optional[str] = None, parameters: Optional[Dict[str, Any]] = None, **kwargs) -> QueryResult:
        """
        Runs the arbitrary Cypher `query` with `parameters` to read data from FalkorDB.
        ...
        Returns:
            Output: Records returned from Cypher query in case request was successful or error message if there was an
                ...
                - `error_message` - Error message returned by FalkorDB in case Cypher query is invalid
                - `error` - Original Exception which was triggered by FalkorDB (containing the `error_message`)
        """
        ...
        try:
            _, records = self._falkordb_client.execute_read(
                query,
            ...
            if self._raise_on_failure:
                logger.error("Couldn't execute FalkorDB read query %s", ex)
                raise ex
    ...
    def _serialize_parameters(self, parameters: Any) -> Any:
        """
        Serializes `parameters` into a data structure which can be accepted by FalkorDB Python Driver (and a Cypher query
        respectively). See \
            [FalkorDBQueryParametersMarshaller][falkordb_haystack.serialization.query_parameters_marshaller.FalkorDBQueryParametersMarshaller]
            for more details.
        """

...
@component
class FalkorDBQueryWriter:
    """
    A component for writing arbitrary data to FalkorDB database using plain Cypher query.

    This component gives flexible way to write data to FalkorDB by running arbitrary Cypher query with
    ...
    The difference between [DocumentWriter](https://docs.haystack.deepset.ai/docs/documentwriter) and `FalkorDBQueryWriter`
    is that the latter can write any data to FalkorDB - not just Documents.
    ...
    ```py title="Example: Creating a Document node with FalkorDBQueryWriter"
    ...
    Important:
        ...
        instances are valid query parameter inputs. However, currently FalkorDB Python Driver does not convert dataclasses
    ...
    def __init__(
        ...
        """
        Create a FalkorDBDocumentWriter component.
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "FalkorDBQueryWriter":
        """
    ...
    def run(self, query: Optional[str] = None, parameters: Optional[Dict[str, Any]] = None, **kwargs) -> QueryResult:
        """
        Runs the arbitrary Cypher `query` with `parameters` to write data to FalkorDB.

        Once data is written to FalkorDB the component returns back some execution stats.
        ...
        try:
            self._falkordb_client.execute_write(
                query,
            ...
            if self._raise_on_failure:
                logger.error("Couldn't execute FalkorDB write query %s", ex)
                raise ex
